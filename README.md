...
* **`on('sync-aborted', Handler:(Provider:LocalStorageProvider, Progress:number) => void)`**<br>the `sync-aborted` event is fired whenever a synchronization between this provider and its associated `Y.Doc` has been aborted (e.g., because the provider was destroyed). `Provider` contains a reference to this provider and `Progress` is always `1.0`
